...
Состояние класса - Полином задается значениями коэффициентов типа double и определяется степенью полинома и значениями коэффициентов,  например: полином  `4*x**2 + 7.5*x - 3.9` задается степенью `2` и значениями коэффициентов `{4.0, 7.5, -3.9}`. Память под полином выделяется статически, во время компиляции, и задается массивом фиксированного предельного значения (например, максимальная степень полинома равна `20`).

Протокол класса - Определяет возможности создания и инициализации экземпляров класса и правила использования их (методы класса).
...
### Замечание
Материалы по полиномам:
- [Википедия](https://en.wikipedia.org/wiki/Polynomial#Division)
- [Алгоритм деления](https://www.cuemath.com/algebra/division-algorithm-for-polynomials/)